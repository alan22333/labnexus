...
1. **资源管理**——文献/链接/文件统一入库、打标签、秒检索(告别"微信文件 7 天过期后重新找导师要");
...
| **F7 资源库** | link/paper/file 三种资源、文件上传(白名单 + 20MB)、标签/类型/关键词筛选 | 本地磁盘存储 |
| **F8 文献元数据** | DOI → Crossref / arXiv ID → arXiv 自动抓取(标题/作者/期刊/年份) | 填 DOI 即得 |
...
| **资源库** | 建链接/文献(填 DOI 自动抓元数据)、上传文件;按类型/关键词筛选 |
